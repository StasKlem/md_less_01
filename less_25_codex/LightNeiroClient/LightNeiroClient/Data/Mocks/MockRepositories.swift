...
actor InMemoryChatStore {
    ...
    var shortTermSnapshot: ShortTermMemorySnapshot?
    var workingMemory: [WorkingMemoryItem] = []
    var longTermMemory: [LongTermMemoryItem] = []
...
struct MockMessageRepository: MessageRepositoryProtocol {
    ...
    func fetchMessages() async throws -> [ChatMessage] {
        (await store.messages).sorted { $0.createdAt < $1.createdAt }
    }

    func saveMessage(_ message: ChatMessage) async throws {
        var current = await store.messages
        current.append(message)
        await store.messages = current
    }
...
struct MockShortTermMemoryRepository: ShortTermMemoryRepositoryProtocol {
    ...
    func fetchSnapshot() async throws -> ShortTermMemorySnapshot? {
        await store.shortTermSnapshot
    }

    func saveSnapshot(_ snapshot: ShortTermMemorySnapshot) async throws {
        await store.shortTermSnapshot = snapshot
    }

    func clear() async throws {
        await store.shortTermSnapshot = nil
    }
}
...
struct MockWorkingMemoryRepository: WorkingMemoryRepositoryProtocol {
    ...
    func fetchActive() async throws -> [WorkingMemoryItem] {
        let current = await store.workingMemory
        return current.filter { $0.status == .active }
    }

    func upsert(items: [WorkingMemoryItem]) async throws {
        var current = await store.workingMemory
        for item in items {
            current.removeAll { $0.key == item.key }
            current.append(item)
        }
        await store.workingMemory = current
    }
...
struct MockLongTermMemoryRepository: LongTermMemoryRepositoryProtocol {
    ...
    func fetch(namespaces: [LongTermMemoryNamespace]?) async throws -> [LongTermMemoryItem] {
        let current = await store.longTermMemory
    ...
    func upsert(items: [LongTermMemoryItem]) async throws {
        var current = await store.longTermMemory
        for item in items {
            current.removeAll { $0.namespace == item.namespace && $0.key == item.key }
            current.append(item)
        }
        await store.longTermMemory = current
    }
...
struct MockMetricsRepository: MetricsRepositoryProtocol {
    ...
    func appendMetric(_ metric: RequestMetric) async throws {
        var current = await store.metrics
        current.append(metric)
        await store.metrics = current
    }
