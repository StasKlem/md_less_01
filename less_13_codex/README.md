## Выбор профиля и влияние на запрос

В настройках есть переключатель профиля пользовательского префикса: `авто`, `профиль 1`, `профиль 2`, `профиль 3`.

Как это работает:
1. Выбранный профиль и тексты всех профилей сохраняются в долговременную память сессии (`namespace: profile`).
2. При отправке сообщения `SendMessageUseCase` запрашивает префикс через `ResolveUserPromptPrefixUseCase`.
3. Берётся текст только выбранного профиля, обрезаются лишние пробелы/переносы.
4. Если текст непустой, пользовательское сообщение превращается в формат: `prefix + пустая строка + user message`.
5. Если текст профиля пустой, сообщение уходит без изменений.

Как это влияет на запрос к LLM:
- В историю ветки и в short-term контекст попадает уже префиксованное сообщение.
- Значит, профиль напрямую меняет формулировку каждого user-turn и поведение модели.
- Рабочая/долговременная память обновляется из исходного `userText` (без префикса), чтобы профиль не засорял извлечение фактов.

## Слои памяти в приложении

В приложении используется 3 типа памяти, каждый со своей задачей и сроком жизни данных.

1. `Краткосрочная память` (текущий диалог)
- Что хранит: последние сообщения активной ветки.
- Где используется: формирование prompt для ближайшего ответа.
- Технически: `slidingWindow` (`windowSize`) или полный контекст ветки в `normal`.
- Срок жизни: пока сообщения есть в истории ветки (в контекст попадает только нужный срез).

2. `Рабочая память` (данные текущей задачи)
- Что хранит: текущая цель, ограничения, промежуточные договорённости, состояние шага.
- Где используется: удержание фокуса в рамках конкретной ветки/задачи.
- Технически: формируется из актуального окна сообщений активной ветки; при смене ветки рабочая память переключается вместе с веткой.
- Срок жизни: до завершения/смены задачи или пока данные остаются релевантными текущему потоку работы.

3. `Долговременная память` (профиль, решения, знания)
- Что хранит: устойчивые факты (`facts`) по `sessionID`:
  - предпочтения пользователя,
  - стабильные правила формата/языка,
  - подтверждённые решения,
  - повторно используемые знания по сессии.
- Где используется: добавляется в контекст при `stickyFacts`.
- Технически: `UpdateFactsUseCase` извлекает и мержит факты в key-value хранилище фактов.
- Срок жизни: на протяжении сессии (дольше, чем рабочий контекст отдельной ветки).

## Как выбирается, что и куда сохранять

Маршрутизация данных выполняется во время отправки сообщения:

1. Сохраняется пользовательское сообщение в активную ветку.
2. Определяется активная стратегия контекста ветки (`normal` / `slidingWindow` / `stickyFacts`).
3. Для краткосрочной и рабочей памяти:
- в контекст берётся либо весь диалог ветки (`normal`), либо последние `N` сообщений (`slidingWindow`, `stickyFacts`);
- это данные «здесь и сейчас», без отдельного долговременного хранения.
4. Для долговременной памяти (только `stickyFacts`):
- берётся последняя пара `user + assistant`;
- LLM в JSON extraction выделяет только устойчивые факты;
- факты мержатся с уже сохранёнными `facts` сессии;
- обновляется технический факт `last-user-message`.
5. В финальный prompt уходят:
- `facts` (если `stickyFacts`),
- актуальный срез сообщений активной ветки.

Правило отбора для долговременной памяти:
- сохранять: то, что пригодится в будущих шагах и не зависит от одного сообщения;
- не сохранять: одноразовые уточнения, шум, дубли, неподтверждённые или конфликтующие формулировки.
