...
struct HackerNewsStoryInputParser {
    func parse(_ rawStory: String) throws -> HackerNewsStoryForTranslation {
        let normalized = rawStory
            .replacingOccurrences(of: "\r\n", with: "\n")
            .trimmingCharacters(in: .whitespacesAndNewlines)

        guard !normalized.isEmpty else {
            throw HackerNewsTranslateToolError.invalidArguments("Story text cannot be empty.")
        }

        let title = value(for: "- Title:", in: normalized)
        guard let title, !title.isEmpty else {
            throw HackerNewsTranslateToolError.invalidArguments("Story must include '- Title: ...'.")
        }
        ...
        return HackerNewsStoryForTranslation(
            id: id,
            title: title,
            author: author,
            score: score,
